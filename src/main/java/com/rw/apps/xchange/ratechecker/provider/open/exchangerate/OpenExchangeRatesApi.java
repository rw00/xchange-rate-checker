...
public class OpenExchangeRatesApi implements ExchangeRateProvider {
    ...
    public OpenExchangeRatesApi(@Value("${provider.open-exchangerates.app-id}") String appId) {
        this.appId = appId;
        restTemplate =
                new RestTemplateBuilder().rootUri("https://openexchangerates.org/api").build();
    }
    ...
    public ExchangeRate getEurToUsdExchangeRate() {
        ...
        BigDecimal rate = BigDecimal.ONE.divide(
                BigDecimal.valueOf(openRate.rates().get(Currency.EURO.getCode())), 2,
