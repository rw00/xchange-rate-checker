...
import com.rw.apps.xchange.ratechecker.db.LastStateDb;
...
public class FxImprovementDetector {
    ...
    private final LastStateDb lastStateDb;

    public FxImprovementDetector(
            @Value("${threshold.base-improvement:0.005}") double baseImprovementThreshold,
            @Value("${threshold.decay-days:4}") int decayDays,
            @Value("${threshold.decay-factor:0.5}") double decayFactor,
            LastStateDb lastStateDb) {
        this.baseImprovementThreshold = baseImprovementThreshold;
        this.decayDays = decayDays;
        this.decayFactor = decayFactor;
        this.lastStateDb = lastStateDb;
    }

    /**
     * Determines if any provider rate in the current snapshot warrants a
     * notification.
     *
     * @param openRate             The current open (mid-market) rate
     * @param providerData         Map of provider names to their current rate and
     *                             calculated spread
     * @param lastNotificationTime The timestamp of the last successful notification
     * @return true if at least one provider shows a superior rate or significant
     *         improvement
     */
    public boolean shouldNotify(double openRate, Map<String, ProviderRateAndSpread> providerData,
        ...
        return providerData.entrySet().stream()
                .map(entry -> processProvider(entry.getKey(), entry.getValue(), openRate, currentThreshold))
    ...
    private double calculateCurrentThreshold(java.time.Instant lastNotificationTime) {
        ...
        long millisPassed = java.time.Duration.between(lastNotificationTime, java.time.Instant.now()).toMillis();
    ...
    private boolean processProvider(String name, ProviderRateAndSpread data, double openRate, double threshold) {
        ...
        if (providerRateVal >= openRate) {
            lastStateDb.updateState(name, currentSpread);
            return true;
        }

        // 2. Significant improvement detection
        ProviderState state = lastStateDb.getOrCreateState(name, currentSpread);
